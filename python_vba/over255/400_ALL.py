...
start_index=f_alist.index("OK") # 修改处1
end_index=f_alist.index("ADP") # 修改处2
...
if __name__=="__main__":
    ...
    into_file("400_ALL",head_str)
    for a_i,num in zip(f_alist[start_index:end_index+1],range(3,end_index-start_index+3+1)):

        f_code=str_1.format(a_i,num,len_str)
        into_file("400_ALL",f_code)
    into_file("400_ALL",tail_next)

    into_file("400_ALL", tail_images.format(f_alist[start_index],f_alist[start_index+149]))
    into_file("400_ALL", tail_images.format(f_alist[start_index+150],f_alist[start_index+249]))
    into_file("400_ALL", tail_images.format(f_alist[start_index+250],f_alist[end_index]))
    into_file("400_ALL",tail_end)

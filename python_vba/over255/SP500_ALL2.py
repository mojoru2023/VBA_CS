...
start_index=f_alist.index("HQ") # 修改处1
end_index=f_alist.index("QB") # 修改处2
...
if __name__=="__main__":
    ...
    into_file("SP500_ALL2",head_str)
    for a_i,num in zip(f_alist[start_index:end_index+1],range(3,end_index-start_index+3+1)):

        f_code=str_1.format(a_i,num,len_str)
        into_file("SP500_ALL2",f_code)
    into_file("SP500_ALL2",tail_next)

    into_file("SP500_ALL2", tail_images.format(f_alist[start_index],f_alist[start_index+100]))
    into_file("SP500_ALL2", tail_images.format(f_alist[start_index+101],f_alist[end_index]))
    into_file("SP500_ALL2",tail_end)
